...
class TestMettmailCLI(TestCase):
    def setUp(self) -> None:
        ...
        self.mock_mettmail_loop = patch("mettmail.mettmail.mettmail_loop", spec_set=mettmail_loop).start()
        self.addCleanup(patch.stopall)

    def test_help(self):
        runner = CliRunner()
        result = runner.invoke(mettmail.run, ["--help"])
        assert result.exit_code == 0
        assert result.output.startswith("Usage:")

    def test_config_not_found(self):
        runner = CliRunner()
        result = runner.invoke(mettmail.run, ["--config", "not-an-existing-file-foo.bar"])
        assert result.exit_code == 1
        assert "No such file" in result.output

    def test_config_incomplete(self):
        runner = CliRunner()
        result = runner.invoke(mettmail.run, ["--config", "tests/data/config.incomplete.yaml"])
        assert result.exit_code == 1
        assert "'envelope_recipient' not found" in result.output

    def test_loglevel_trace(self):
        runner = CliRunner()
        result = runner.invoke(mettmail.run, ["--trace", "--config", "not-an-existing-file-foo.bar"])
        assert result.exit_code == 1
        assert "level=5" in str(logger)  # hacky but loguru doesn't have a way to request the loglevel

    def test_loglevel_debug(self):
        runner = CliRunner()
        result = runner.invoke(mettmail.run, ["--debug", "--config", "not-an-existing-file-foo.bar"])
        assert result.exit_code == 1
